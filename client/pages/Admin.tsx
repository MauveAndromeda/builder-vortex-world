    ...
    <section className="py-10">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={clear} className="rounded border px-3 py-1 text-sm">
            Sign out
          </button>
        </div>
      </div>
      <div className="mt-4 flex gap-2 text-sm">
        <button
          ...
          className={`rounded-full px-3 py-1 ${tab === "content" ? "bg-foreground text-background" : "border"}`}
        >
          Content
        </button>
        <button
          onClick={() => setTab("stats")}
          className={`rounded-full px-3 py-1 ${tab === "stats" ? "bg-foreground text-background" : "border"}`}
        >
          Stats
        </button>
        <button
          onClick={() => setTab("ai")}
          className={`rounded-full px-3 py-1 ${tab === "ai" ? "bg-foreground text-background" : "border"}`}
        >
          AI Tools
        </button>
      </div>
...
function AdminContent({ token }: { token: string }) {
  const [list, setList] = useState<{ works: any[]; featured: string[] }>({
    works: [],
    featured: [],
  });
  ...
  async function del(slug: string) {
    const r = await fetch(`/api/admin/works/${encodeURIComponent(slug)}`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${token}` },
    });
    if (r.ok) load();
  ...
  async function toggleFeatured(slug: string, add: boolean) {
    const r = await fetch(`/api/admin/works/${encodeURIComponent(slug)}`, {
      ...
      body: JSON.stringify({ featured: add }),
    });
    if (r.ok) load();
  }
  ...
  async function autoSplit() {
    ...
    const r = await fetch("/api/admin/chapters/split", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
    ...
    <div className="mt-6 grid gap-8 md:grid-cols-2">
      <form onSubmit={upload} className="rounded-2xl border p-4 space-y-2">
        ...
        <textarea
          ...
          placeholder="Full content (for auto chapters)"
        ...
        <div className="mt-3 space-y-3">
          {list.works.map((w) => (
            ...
            >
              ...
              <button
                onClick={() =>
                  toggleFeatured(w.slug, !list.featured.includes(w.slug))
                }
                className="rounded border px-2 py-1 text-xs"
              >
                {list.featured.includes(w.slug) ? "Unfeature" : "Feature"}
              </button>
              <button
                onClick={() =>
                  setPrice(
                    w.slug,
                    Number(prompt("New price", String(w.price ?? 0)) || 0),
                  )
                }
                className="rounded border px-2 py-1 text-xs"
              >
...
function AdminAI({ token }: { token: string }) {
  ...
  async function run() {
    const r = await fetch("/api/admin/ai/optimize", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
    ...
    <div className="mt-6 grid gap-4 md:grid-cols-2">
      <div className="rounded-2xl border p-4">
        ...
        <textarea
          ...
          className="w-full h-64 rounded border px-3 py-2 mt-2"
        />
        <button
          onClick={run}
          className="mt-2 rounded bg-foreground text-background px-4 py-2"
        >
          Optimize
        </button>
      </div>
