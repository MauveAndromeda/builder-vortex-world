...
export default function AccountLayout() {
  ...
  function signOut() {
    ...
    setUser(null);
    nav(localized("/account/sign-in", locale));
  }
  return (
    <section className="py-16">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <Link
            to={localized("/checkout/donation", locale)}
...
export function SignIn() {
  ...
  function submit(e: React.FormEvent) {
    ...
    const u = getUser();
    if (!u || !saved) {
      setErr("No account found. Please register.");
      return;
    }
  ...
  return (
    <form onSubmit={submit} className="max-w-sm grid gap-3">
      ...
      <button className="mt-1 rounded-full bg-foreground text-background px-4 py-2 text-sm">
        Sign in
      </button>
    </form>
  );
...
export function Purchases() {
  const owned = useMemo(
    () => works.filter((w) => localStorage.getItem(`owned:${w.slug}`) === "1"),
    [],
  );
  return (
    <div className="grid gap-3">
      ...
      <div className="rounded-2xl border p-4">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          If you enjoy the reading experience, consider a small donation.
        </p>
        <Link
          to="../checkout/donation"
          className="mt-3 inline-flex rounded-full bg-foreground text-background px-4 py-2 text-sm"
        >
          Donate
        </Link>
      </div>
