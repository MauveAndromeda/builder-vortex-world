  ...
  return (
    <svg className="absolute inset-0 w-full h-full" aria-hidden>
      ...
      {Array.from({ length: 120 }).map((_, i) => (
        <circle key={i} cx={`${Math.random()*100}%`} cy={`${Math.random()*100}%`} r={Math.random()*1.5+0.3} fill="white" opacity={Math.random()*0.9+0.1}>
          <animate attributeName="opacity" values="0.5;1;0.5" dur={`${0.6 + Math.random()*0.6}s`} repeatCount="indefinite" begin={`${Math.random()}s`} />
        </circle>
      ))}
      <g>
      ...
      <g>
        <line x1="-10%" y1="20%" x2="20%" y2="10%" stroke="#fff" strokeWidth="2">
          <animate attributeName="x1" values="-10%;110%" dur="22s" repeatCount="indefinite" />
          <animate attributeName="y1" values="20%;-10%" dur="22s" repeatCount="indefinite" />
          <animate attributeName="x2" values="20%;140%" dur="22s" repeatCount="indefinite" />
          <animate attributeName="y2" values="10%;-40%" dur="22s" repeatCount="indefinite" />
        </line>
      </g>
    </svg>
  );
