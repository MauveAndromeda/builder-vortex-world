...
export default function Starfield({ className = "", avoidRects = [] as DOMRect[] }: { className?: string; avoidRects?: DOMRect[] }) {
  ...
  useEffect(() => {
    ...
    const makeStars = (count: number, rMin: number, rMax: number) => Array.from({ length: count }, () => ({
      x: Math.random() * w,
      y: Math.random() * h,
    ...
    function remapAvoidRects(): Rect[] {
      ...
      return avoidRects.map(r => ({ top: r.top - canvasRect.top, bottom: r.bottom - canvasRect.top }));
    }

    function pickSafeY(ranges: Rect[]): number {
      // Exclude avoid ranges (expanded) from [0, h*0.6]
      const limit = h * 0.6;
      const expanded = ranges.map(r => ({ top: Math.max(0, r.top - 24), bottom: Math.min(limit, r.bottom + 24) }))
      ...
      const choice = holes.length ? holes[Math.floor(Math.random() * holes.length)] : [0, limit] as [number, number];
    ...
    function drawMeteor() {
      if (!meteor) return;
      const m = meteor; m.x += m.vx; m.y += m.vy; m.life *= 0.965;
      ctx.strokeStyle = `rgba(255,255,255,${m.life})`;
      ctx.lineWidth = 2;
      ctx.beginPath(); ctx.moveTo(m.x, m.y); ctx.lineTo(m.x - 26, m.y - 10); ctx.stroke();
      if (m.life < 0.06 || m.x > w + 40 || m.y > h + 40) meteor = null;
    }

    function drawLayer(stars: { x: number; y: number; r: number; tw: number }[], t: number, parallax: number) {
      const offset = Math.min(24, scrollY * parallax);
      for (const s of stars) {
        const a = 0.45 + 0.55 * (0.5 + 0.5 * Math.sin(t / 900 + s.tw));
        ctx.globalAlpha = a;
        ctx.fillStyle = "#fff";
        ctx.beginPath(); ctx.arc(s.x, s.y + offset, s.r, 0, Math.PI * 2); ctx.fill();
      }
      ctx.globalAlpha = 1;
    }
    ...
    const onResize = () => {
      ...
      const d = Math.min(2, window.devicePixelRatio || 1); canvas.width = w * d; canvas.height = h * d; ctx.scale(d, d);
    };
    ...
    const loop = (t: number) => {
      ...
      g.addColorStop(0, "#0a1b3f"); g.addColorStop(1, "#152a5c");
    ...
    setTimeout(() => { raf = requestAnimationFrame(loop); }, 0);
