...
export function attachAdmin(app: any) {
  ...
  app.get(
    ...
    (_req: Request, res: Response) => {
      res.json({
        works: store.works,
        featured: Array.from(store.featuredSlugs),
      });
    },
  );
  ...
  app.get(
    ...
    (_req: Request, res: Response) => {
      res.json({
        ...
        featuredCount: store.featuredSlugs.size,
      });
    },
  );
  ...
  app.post(
    ...
    (req: Request, res: Response) => {
      ...
      if (!title || !slug)
        return res.status(400).json({ error: "missing_title_or_slug" });
    ...
    (req: Request, res: Response) => {
      try {
        ...
        if (!dataUri || typeof dataUri !== "string")
          return res.status(400).json({ error: "missing_dataUri" });
